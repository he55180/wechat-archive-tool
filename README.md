...
> 核心链路（抓取→排版→归档）零外部AI依赖，完全离线可用，不需要任何 API Key 即可完整运行。
>
> 在此基础上，新增两个**可选**的 AI 增值层，默认关闭，需自行在 `config.json` 中配置 API Key 才会启用：
> - **AI 摘要**：为归档文章生成一句话摘要，写入 Front Matter 的 `summary` 字段
> - **AI 增强分类**：当关键词规则判断不明确时（仅在标题+正文开头完全未命中任何分类关键词的情况，约占样本的 14.7%），调用 AI 做二次裁决，辅助归档到更准确的分类文件夹
>
> 两个 AI 功能均已做容错降级：未配置、网络失败、超时、返回格式异常等任何情况，都会静默跳过并回退到规则/空值，不会中断或阻塞主流程。
...
**Q：AI 摘要 / AI 分类功能没配置 API Key，或者 API 调用失败会怎样？**
A：完全不影响使用。这两个功能都是可选的增值层：
- 未配置 API Key：功能自动跳过，`summary` 字段留空，分类走原有规则引擎判断，归档流程不受影响。
- API 调用失败或超时：自动捕获异常并降级——摘要跳过、分类回退到规则引擎的判断结果，同时在日志/终端输出警告信息，不会导致抓取或归档失败。
- AI 增强分类还内置了熔断机制：单次运行任务中如果连续多次调用失败，会自动关闭 AI 判断，后续文章直接走规则引擎，避免逐篇卡在超时等待上。
...
| v2.0.0 | GUI 图形界面 + 标点修正引擎（format_expert.py） + Pandoc 安装引导优化（自动检测/兜底安装） + 首次启动自动创建桌面快捷方式 |
