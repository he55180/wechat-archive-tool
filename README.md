...
> - **AI 增强分类**：当关键词规则判断不明确时（仅在标题+正文开头完全未命中任何分类关键词的情况，约占样本的 17.6%），调用 AI 做二次裁决，辅助归档到更准确的分类文件夹
>
...
- AI 增强分类还内置了熔断机制：同一批处理任务中如果连续多次调用失败，会自动关闭 AI 判断，后续文章直接走规则引擎，避免逐篇卡在超时等待上。
